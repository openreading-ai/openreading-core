...
async def test_stdio_import_search_read_and_sanitized_invalid_arguments(tmp_path):
    ...
    async with stdio_client(params) as (reader, writer), ClientSession(reader, writer) as session:
        ...
        assert 'delivery="auto"' in guidance
        assert "host-created file" in guidance
        assert "local path does not establish cloud access" in guidance
        assert "Do not claim the whole result was read" in guidance
        ...
        assert json.loads(unavailable.content[0].text)["error"]["code"] == "selection_unavailable"
