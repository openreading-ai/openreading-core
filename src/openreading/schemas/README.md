...
`fixtures: 0 checked` is expected today (see Known gaps). From Python, call
`validate_<family>(instance)`, for example `validate_response`. The families are the rows of the
table below. Each function returns `None` or raises `jsonschema.ValidationError`.
...
if n.endswith('_SCHEMA_FILE')})"`. If this table and that output disagree, the output is right. Fix
the table.
...
Each family's `$id`, and the version marker inside an instance:
...
indent=1))"`. If this table and that output disagree, the output is right. Fix the table.
...
`warnings[].code` is an open set: switch on the codes you know and tolerate the rest. The first
string argument of every `add_warning(` call is a code. List today's with:
...
HTTP 400. Response history: `CHANGELOG.md` and the "Versioning rules" section of `uv run python -m pydoc
openreading.schemas`.

## Notes

- `document.page_count` is the source page count even when you request a subset. Check: `uv run
  openreading parse sample.pdf --backend pymupdf --pages 2` prints `page_count: 2` and one page.
...
- A `pymupdf` run also carries `usage`, `backend_raw` and `channel_provenance` today. Check: `uv run
  openreading parse sample.pdf --backend pymupdf | python3 -c "import json,sys;
  print(list(json.load(sys.stdin)))"`.

## Known gaps

- `openreading.SCHEMA_VERSION` prints `0.1`, the request family's number, unlabelled. Reproduce: `uv
  run python -c "import openreading; print(openreading.SCHEMA_VERSION)"`.
- `validate` reports `fixtures: 0 checked` because its fixture glob names a directory that does not
  exist. Reproduce: `uv run python -m openreading.schemas validate`.
- An out-of-range page range returns the whole document with no warning. Reproduce: `uv run
  openreading parse sample.pdf --backend pymupdf --pages 99` (exit 0, both pages, only
...
- [Backend adapters](../adapters/README.md) — every backend, its env vars and compliance posture.
- [`tests/test_schema_evolution.py`](../../../tests/test_schema_evolution.py) — the byte pins on
  older files.
...
`openreading.types`. Add a `CHANGELOG.md` line. Leave the old file untouched: its hash is pinned. A
new required field, enum value or const goes in the response table above. A new warning code needs
no edit here. When a Known-gaps line stops being true, delete it. The full table of what to update
for each kind of change is under *Where a change gets documented* in
[`AGENTS.md`](../../../AGENTS.md).
